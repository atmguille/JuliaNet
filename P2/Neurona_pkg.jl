...
"""
    __sigmoide(x::Float64)

Función sigmoide
# Arguments:
- `x::Float64`: Valor a evaluar

"""
__sigmoide(x::Float64) = 2.0 / (1.0 + exp(-x)) - 1.0
...
function Disparar(neurona::Neurona)
    ...
    elseif neurona.tipo == Sigmoide
        neurona.valor_salida = __sigmoide(neurona.valor_entrada)
    end
end
...
function DerivadaActivacion(neurona::Neurona)
    if neurona.tipo == Sigmoide
        sigmoide_eval = __sigmoide(neurona.valor_entrada)
        return 0.5 * (1 + sigmoide_eval) * (1 - sigmoide_eval)
    else
        println("[!] No se ha definido la derivada para este tipo de neurona")
        return 0.0
    end
end
